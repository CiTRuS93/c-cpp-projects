    ...
    */
    ...
    for (size_t i = 0; i < header->e_shnum; i++){
        if(sh[i].sh_type == SHT_REL ){
           ...
           char * relStr = (char*)(map_start + sh[i+1].sh_offset);
        ...
        if(sh[i].sh_type == SHT_REL ){
           ...
           char * relStr = (char*)(map_start + sh[i+1].sh_offset);
            ...
            for(int j=0;j<relSh->sh_size/sizeof(Elf32_Rel);j++){
                ...
                fprintf(stdout,"%x\t%x\t%x\t%s\n",rel[j].r_offset,rel[j].r_info,dyn[id].st_value,&symStr[dyn[id].st_name]);
            }
        }
...
int main(int argc, char **argv){
    ...
    struct fun_desc menu[] = { { "Toggle Debug Mode",toggle_debug_mode}, { "Examine ELF File",exemainElf},{"Print Section Names",SecName},{"Print Symbols",printSym},
        {"Link check",linkChk},{"Relocation Tables",relTable},{ "Quit", quit}, {NULL,NULL} };
