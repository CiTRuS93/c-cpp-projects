...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    ...
    fseek(inf,signitureOffset,SEEK_SET);
    fwrite(NOP,1,signitureSize,inf);
    fclose(inf);
}
void fix(){
    ...
    *n='\0';
    ...
    signitureOffset = strtol(input,NULL,10);
    printf("enter size: \n");
    fgets(input,256,stdin);
    signitureSize = strtol(input,NULL,10);
    kill_virus(path,signitureOffset,signitureSize);
